...
class CartpoleIsaacRollout(RolloutBase):
    def __init__(self, dynamics_model, tensor_args={'device':'cpu','dtype':torch.float32}):
        super(CartpoleIsaacRollout, self).__init__()
        self.dynamics_model = dynamics_model
        self.tensor_args = tensor_args

    def cost_fn(self, state, act):
        slider_pos = state[:, :, 0]
        pole_ang = angle_normalize(state[:, :, 2])
        slide_vel = state[:, :, 1]
        pole_vel = state[:, :, 3]

        cost = 0.1*slider_pos ** 2
        cost += pole_ang ** 2
    ...
    def rollout_fn(self, start_state, act_seq):
        trajectories = self.dynamics_model.rollout_open_loop(start_state, act_seq)
